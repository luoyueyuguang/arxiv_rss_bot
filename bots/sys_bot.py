...
class SysBot(BaseConferenceBot):
    ...
    HIGH_SCORE_KEYWORDS = [
        ...
        # AI Infra — Serving & Systems
        "AI agent", "agent serving", "agent system", "multi-agent serving",
        "tool calling", "function call serving",
        "prompt caching", "prefix caching", "automatic prefix caching",
        "structured output", "guided decoding", "constrained decoding",
        "grammar-constrained",
        "token healing", "speculative sampling", "Medusa", "Eagle",
        "lookahead decoding", "restart decoding",
        "attention optimization", "paged attention", "radix attention",
        "cascade attention", "sparse attention", "sliding window",
        "ring attention", "tree attention",
        "KV cache compression", "KV cache quantization",
        "KV cache eviction", "KV cache offloading",
        "model parallelism", "tensor parallelism",
        "sequence parallelism", "context parallelism",
        "ZeRO", "activation checkpointing", "gradient accumulation",
        "micro-batching", "pipeline bubble",
        "automatic mixed precision", "AMP", "bfloat16",
        "NCCL", "NVLink", "NVSwitch", "GPU direct", "GPU RDMA",
        "SHARP", "network offload",
        "disaggregated prefill", "splitwise", "Sarathi",
        "split infer", "prefill-decode disaggregation",
        # Agent / Infra Systems
        "RAG serving", "vector database", "embedding serving",
        "reranker", "hybrid search",
        "agent workflow", "LLM orchestration",
        "LangChain", "LlamaIndex", "flow engineering",
        "prompt optimization", "DSPy", "auto-prompt",
        "agent framework",
    ]
